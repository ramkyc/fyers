...
def fetch_and_store_historical_data(fyers: fyersModel.FyersModel, symbols: list, resolutions: list):
    ...
    """
    print(f"Connecting to database at: {config.HISTORICAL_MARKET_DB_FILE}")
    con = sqlite3.connect(config.HISTORICAL_MARKET_DB_FILE)
    cursor = con.cursor()

    # Drop the table if it exists to ensure a clean schema
    cursor.execute(f"DROP TABLE IF EXISTS {HISTORICAL_TABLE};")
