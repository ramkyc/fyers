...
class PT_Portfolio:
    ...
    def execute_order(self, symbol: str, timeframe: str, action: str, quantity: int, price: float, timestamp: datetime):
        ...
        Args:
            ...
            timeframe (str): The timeframe on which the signal was generated (e.g., '15', 'D').
    ...
    def _update_position(self, symbol, timeframe, quantity, price, timestamp, action):
        ...
        if quantity > 0 and position_key not in self.position_capital:
            self.position_capital[position_key] = 0 # It will be updated by the trade value
        ...
        print(f"{timestamp} | Trade on {timeframe}: {'BUY' if quantity > 0 else 'SELL'} {abs(quantity)} {symbol} @ {price:.2f}")
    ...
    def get_position(self, symbol: str, timeframe: str):
        ...
        Args:
            symbol (str): The stock symbol.
            timeframe (str): The timeframe of the position.
    ...
    def get_capital_for_position(self, symbol: str, timeframe: str, default_trade_value: float) -> float:
        ...
        If no capital has been allocated yet, it uses the default trade value.
        """
        position_key = (symbol, timeframe)
        
        # If this is the first trade for this slot, allocate the default capital.
